...
import nl.enovation.addressbook.cqrs.webui.contacts.ContactsController;
...
public class PhoneNumberControllerIntegrationTest {
    ...
    public void testDeletePhoneNumber() {
        ...
        PhoneNumberEntry phoneNumber = createPhoneNumberEntry();
        CreatePhoneNumberCommand setupPhoneNumberCommand = new CreatePhoneNumberCommand(new StringAggregateIdentifier(contactEntry.getIdentifier()), phoneNumber);
        commandBus.dispatch(setupPhoneNumberCommand);
        assertEquals("ContactEntry should have the new phoneNumber set in the database", 1, contactQueryRepository.findOne(contactEntry.getIdentifier()).getPhoneNumbers().size());
...
//
//        // Check that we'reback to the original form
//        assertEquals("phonenumbers/edit", view);
//    }
//
//    @Test
